...
export default function Overview({ reels, stories, onOpenReel, m }) {
  ...
  return (
    <div>
      ...
      {/* Today's content */}
      <Section m={m} label={`On the plan today (${todays.length} reel${todays.length === 1 ? "" : "s"}${todayStories.length ? ` · ${todayStories.length} story day${todayStories.length === 1 ? "" : "s"}` : ""})`}>
        {todays.length === 0
          ...
          : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {todays.map(r => <ReelRow key={r.id} m={m} onOpenReel={onOpenReel} r={r} right={<Chip text={STATUS_LABEL[r.status]} color={STATUS_COLOR[r.status]} />} />)}
            </div>}
      </Section>

      {/* Month stats */}
      <Section m={m} label={`This month · ${monthName}`}>
      ...
      {creatorOrder.length > 0 && (
        <Section m={m} label="Creators — this month">
      ...
      <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr", gap: m ? 0 : 20 }}>
        <Section m={m} label="Recently posted">
          {posted.length === 0
            ...
            : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {posted.map(r => <ReelRow key={r.id} m={m} onOpenReel={onOpenReel} r={r} right={m && r.assignee ? <Chip text={r.assignee} color={aColor(r.assignee)} /> : null} />)}
              </div>}
        </Section>
        <Section m={m} label="Coming up next">
          {upcoming.length === 0
            ...
            : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {upcoming.map(r => <ReelRow key={r.id} m={m} onOpenReel={onOpenReel} r={r} right={<Chip text={STATUS_LABEL[r.status]} color={STATUS_COLOR[r.status]} />} />)}
              </div>}
        </Section>
      </div>
    </div>
...
// Module-level so they keep their identity between renders (a component
// defined inside the parent remounts its subtree on every render).
function Section({ label, children, style, m }) {
  return (
...
function ReelRow({ r, right, m, onOpenReel }) {
  return (
